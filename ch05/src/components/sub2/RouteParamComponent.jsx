...
function RouteParamComponent() {
  ...
  const changeHandler = (e) => {
    setFormData({ ...formData, [e.target.name]: e.target.value }); //formData에 name속성값을 value로 업데이트
  };
  ...
  return (
    <div className="RouteParamComponent">
      ...
      <form>
        ...
        />{" "}
        <br />
        <input type="button" value="쿼리 스트링" onClick={btn1Handler} /> <br />
        <input
          type="button"
          value="주소 파라미터 전송"
          onClick={btn2Handler}
        />{" "}
        <br />
        <input type="button" value="State 전송" onClick={btn3Handler} /> <br />
      </form>
      <Routes>

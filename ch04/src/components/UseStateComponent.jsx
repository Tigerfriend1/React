...
function UseStateComponent(){
    ...
    const [count, setCount] = useState(0);  //숫자(count)
    const [text, setText] = useState("");   //문자(text)
    const [user, setUser] = useState({      //객체(user)
                                        uid : "",
                                        name : "",
                                        age : 0
                                    });
    ...
    return (
        <div className="UsestateComponent">
            ...
            <input type="text" name="uid" value={user.uid} onChange={(e)=>{setUser({...user,uid:e.target.value})}} /><br/>
